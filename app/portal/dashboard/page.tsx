    ...
    <main className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-brand-card border border-brand-border rounded-xl p-5">
          <div className="text-brand-muted text-xs font-semibold uppercase tracking-wider mb-2">Outstanding Balance</div>
          <div className="text-3xl font-extrabold text-gradient">
            ${balance.toLocaleString('en-US', { minimumFractionDigits: 2 })}
          </div>
        </div>
        <div className="bg-brand-card border border-brand-border rounded-xl p-5">
          <div className="text-brand-muted text-xs font-semibold uppercase tracking-wider mb-2">Open Invoices</div>
          <div className="text-3xl font-extrabold text-white">
            {invoices.filter((i) => i.status !== 'paid').length}
          </div>
        </div>
        <div className="bg-brand-card border border-brand-border rounded-xl p-5">
          <div className="text-brand-muted text-xs font-semibold uppercase tracking-wider mb-2">Overdue</div>
              ...
              <thead>
                <tr className="border-b border-brand-border">
                  {['Invoice #', 'Due Date', 'Amount', 'Balance Due', 'Status', 'Action'].map((h) => (
                    <th key={h} className="px-6 py-3 text-left text-xs font-semibold text-brand-muted uppercase tracking-wider">
                      {h}
                    </th>
                  ))}
                </tr>
